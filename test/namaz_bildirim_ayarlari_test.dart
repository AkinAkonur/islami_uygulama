...
void main() {
  ...
  late NotificationPlatformFake notificationFake;

  setUp(() async {
    SharedPreferences.setMockInitialValues({});
    await NamazBildirimAyarlari.sifirla();
    GercekBildirimler.testDurumunuSifirla();
    notificationFake = NotificationPlatformFake()..install();
  });

  tearDown(() => notificationFake.uninstall());
  ...
  group('NamazBildirimAyarlariPage', () {
    setUp(() async { await vakitOnbellegiHazirla(); });
    ...
    testWidgets('vakit seçeneği değiştirilince kaydedilir', (tester) async {
      ...
      expect(NamazBildirimAyarlari.dakikaOnce(NamazVakti.imsak), 45);
      expect(find.text('İmsak Namazı: 45 dk önce bildirilecek'), findsOneWidget);
    });
    ...
    testWidgets('test bildirimi butonu güvenle çalışır', (tester) async {
      ...
      expect(notificationFake.calls.where((c) => c.method == 'show'), hasLength(1));
      expect(notificationFake.calls.where((c) => c.method == 'zonedSchedule'), hasLength(1));
      expect(find.byType(SnackBar), findsOneWidget);
      expect(find.text('Test bildirimi 5 saniye içinde gelecek'), findsOneWidget);
    });
  });
}