...
class RadyoOynaticiStore {
  ...
  static Future<void> baslat({List<RadyoKanali>? kanallar}) async {
    ...
    player.processingStateStream.listen((durum) {
      if (durum == ProcessingState.ready ||
          durum == ProcessingState.idle ||
  ...
  static Future<void> _oynatGuvenli() async {
    try {
      await player.play();
    } catch (_) {
      yukleniyor.value = false;
      calyor.value = false;
      hata.value = 'Bu radyo akışına ulaşılamadı. Bağlantınızı kontrol edip tekrar deneyin.';
    }
  }
