...
class AyarlarStore {
  ...
  static const _geminiKey = 'ayar_gemini_api_key';
  ...
  /// Cihazda saklanan Gemini anahtarı (asla sunucuya/ZIP'e yazılmaz).
  static final ValueNotifier<String> geminiApiAnahtari = ValueNotifier<String>('');
  ...
  static Future<void> baslat() async {
    ...
    geminiApiAnahtari.value = (p.getString(_geminiKey) ?? '').trim();
  }
  ...
  static Future<String> geminiAnahtarOku() async {
    final v = ((await _p).getString(_geminiKey) ?? '').trim();
    geminiApiAnahtari.value = v;
    return v;
  }

  /// Anahtar yalnızca bu cihazın SharedPreferences'ında tutulur.
  /// Sohbete, ZIP'e veya kaynak koda yazılmaz.
  static Future<void> geminiAnahtarYaz(String? deger) async {
    final p = await _p;
    final temiz = (deger ?? '').trim();
    if (temiz.isEmpty) {
      await p.remove(_geminiKey);
      geminiApiAnahtari.value = '';
    } else {
      await p.setString(_geminiKey, temiz);
      geminiApiAnahtari.value = temiz;
    }
  }

  static bool get geminiAnahtarVar => geminiApiAnahtari.value.trim().isNotEmpty;
}