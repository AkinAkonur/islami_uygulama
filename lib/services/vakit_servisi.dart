...
class VakitTakvimi {
  final List<VakitBilgisi> vakitler;
  final String saatDilimi;
  const VakitTakvimi(this.vakitler, this.saatDilimi);
}
...
class VakitServisi {
  ...
  static const _keyDogrulanmis = 'vakitler_dogrulanmis_v2';
  static const _keySaatDilimi = 'vakitler_saat_dilimi';
  ...
  static final ValueNotifier<bool> guncelVeriVar = ValueNotifier<bool>(false);
  ...
  static Future<List<VakitBilgisi>> gunlukVakitler() async {
    ...
    if (p.getString(_keyVakitGun) == bugun && p.getBool(_keyDogrulanmis) == true) {
      ...
      if (raw != null && raw.isNotEmpty) {
        try {
          ...
          final vakitler = list
              ...
              .toList();
          if (vakitler.length != 6) throw const FormatException('Eksik vakit');
          guncelVeriVar.value = true;
          return vakitler;
        } catch (_) {
  ...
  static Future<void> _cacheTemizle() async {
    ...
    await p.remove(_keyDogrulanmis);
    await p.remove(_keySaatDilimi);
  }
  ...
  static Future<List<VakitBilgisi>> _cikar() async {
    try {
      final gun = await _apiGetir(DateTime.now());
      final p = await _p;
      await p.setString(_keyVakitler,
          jsonEncode(gun.vakitler.map((v) => v.toJson()).toList()));
      await p.setString(_keyVakitGun, _tarihKey(DateTime.now()));
      await p.setString(_keySaatDilimi, gun.saatDilimi);
      await p.setBool(_keyDogrulanmis, true);
      guncelVeriVar.value = true;
      return gun.vakitler;
    } catch (e) {
      // Görsel yedek saatleri gerçek API verisi olarak kaydetme.
      // Bildirim planlaması bu örnek listeyi ASLA kullanmaz.
      debugPrint('[Vakit] Güncel veri alınamadı: $e');
      guncelVeriVar.value = false;
      return varsayilan;
    }
  }

  /// Yalnızca tarihe ait gerçek veri; ağ yoksa sahte vakit üretmez.
  static Future<VakitTakvimi> bildirimTakvimi(DateTime tarih) async {
    final p = await _p;
    if (p.getBool(_keyDogrulanmis) == true &&
        p.getString(_keyVakitGun) == _tarihKey(tarih)) {
      try {
        final raw = jsonDecode(p.getString(_keyVakitler)!) as List;
        final liste = raw.map((v) => VakitBilgisi.fromJson(
          (v as Map).cast<String, dynamic>())).toList();
        final bolge = p.getString(_keySaatDilimi);
        if (liste.length == 6 && bolge != null && bolge.isNotEmpty) {
          return VakitTakvimi(liste, bolge);
        }
      } catch (_) {}
    }
    return _apiGetir(tarih);
  }

  static Future<VakitTakvimi> _apiGetir(DateTime tarih) async {
    final bugun = _tarihApi(tarih);
    ...
    if (koordinat != null) {
      url = Uri.parse(
        'https://api.aladhan.com/v1/timings/$bugun'
      ...
      url = Uri.parse(
        'https://api.aladhan.com/v1/timingsByCity/$bugun'
