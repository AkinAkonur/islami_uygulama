...
static const List<({String kod, String ad})> _metotlar = [
  ...
  Future<void> _yukle() async {
    ...
    final gemini = await AyarlarStore.geminiAnahtarOku();
    if (!mounted) return;
    setState(() {
      ...
      _metotKod = metot ?? '';
      _vurguKod = vurgu;
      _geminiKayitli = gemini.isNotEmpty;
    });
  }
  ...
  Future<void> _geminiAnahtarSec() async {
    ...
    await AyarlarStore.geminiAnahtarYaz(sonuc);
    if (!mounted) return;
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          if (kDebugMode) ...[
            const SizedBox(height: 20),
            _bolumBasligi(l.t('set.aiSection')),
            _ayarSecenegi(
              Icons.auto_awesome,
              l.t('set.aiKey'),
              altMetin: _geminiKayitli
                  ? l.t('set.aiKeyStatusOn')
                  : l.t('set.aiKeyStatusOff'),
              onTap: _geminiAnahtarSec,
            ),
          ],
