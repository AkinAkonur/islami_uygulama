...
class _NamazBildirimAyarlariPageState extends State<NamazBildirimAyarlariPage> {
  ...
  Future<void> _sesDegistir(BildirimSesi? deger) async {
    if (deger == null) return;
    await NamazBildirimAyarlari.sesAyarla(deger);
    // Android ses özelliklerini kanal bazında sakladığı için yeni seçimden
    // sonra tüm vakitleri yeni kanal kimliğiyle yeniden planlamak gerekir.
    await GercekBildirimler.planla();
  }

  String _sesAdi(BildirimSesi ses, AppLocalizations l) => switch (ses) {
        BildirimSesi.ezanKisa => l.t('nba.soundAdhan'),
        BildirimSesi.altinCingilti => l.t('nba.soundGold'),
        BildirimSesi.zumrutDamla => l.t('nba.soundEmerald'),
        BildirimSesi.huzurZili => l.t('nba.soundPeace'),
        BildirimSesi.sessiz => l.t('nba.soundSilent'),
      };
  ...
  Future<void> _dakikaSec(NamazVakti vakit, int? dakika) async {
    ...
    final mesaj = dakika < 0
        ? l.t('nba.disabled').replaceFirst('{v}', l.vakitAdi(vakit.ad))
        : dakika == 0
            ? l.t('nba.onTimeSnack').replaceFirst('{v}', l.vakitAdi(vakit.ad))
            : l.t('nba.minutesBefore')
                .replaceFirst('{v}', l.vakitAdi(vakit.ad))
  ...
  Future<bool> _sistemAyariAc(String metod) async {
    try {
      const channel = MethodChannel('com.akinakonur.islamiuygulama/test');
      return await channel.invokeMethod<bool>(metod) ?? false;
  ...
  Future<void> _pilOptimizasyonuIste() async {
    if (!await _sistemAyariAc('openBatterySettings')) {
      await _uygulamaAyarlari();
    }
  }

  Future<void> _uygulamaAyarlari() async {
    if (!await _sistemAyariAc('openAppSettings') && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
    ...
    return _kart(
      child: Column(
        ...
        children: [
          Row(
            children: [
              ...
              Switch(
                ...
                activeTrackColor: Renkler.acikZumrutSabit,
              ),
            ],
          ),
          ...
          const Divider(height: 24, color: Colors.white12),
          ValueListenableBuilder<BildirimSesi>(
            valueListenable: NamazBildirimAyarlari.ses,
            builder: (context, seciliSes, _) => Row(
              children: [
                const UcdIkon(
                  ikon: Icons.volume_up_rounded,
                ...
                Flexible(
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<BildirimSesi>(
                      value: seciliSes,
                      ...
                      items: [
                        for (final ses in BildirimSesi.values)
                          DropdownMenuItem<BildirimSesi>(
                            value: ses,
                            child: Text(_sesAdi(ses, l)),
                          ),
                      ],
                      onChanged: _master ? _sesDegistir : null,
                    ),
                  ),
                ),
              ],
      ...
      child: Column(
        ...
        children: [
          ...
          FilledButton.icon(
            ...
            icon: const UcdIkon(ikon: Icons.notifications_active_rounded, renk: Renkler.acikZumrutSabit, boyut: 18),
            label: Text(l.t('nba.sendTest')),
            style: FilledButton.styleFrom(
              backgroundColor: Renkler.acikZumrutSabit.withValues(alpha: 0.2),
              foregroundColor: Renkler.acikZumrutSabit,
            ),
          ),
        ],
      ),
          ...
          Row(
            children: [
              UcdIkon(ikon: Icons.battery_alert_rounded,
                  renk: Renkler.acikAltinSabit, boyut: 20),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
          ...
          Wrap(
            ...
            children: [
              OutlinedButton.icon(
                ...
                icon: const UcdIkon(ikon: Icons.battery_charging_full_rounded, renk: Renkler.acikAltinSabit, boyut: 16),
                label: Text(l.t('nba.batteryDisable')),
                style: OutlinedButton.styleFrom(
                  foregroundColor: Renkler.acikAltinSabit,
