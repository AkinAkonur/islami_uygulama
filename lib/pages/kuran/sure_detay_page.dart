...
class _SureDetayPageState extends State<SureDetayPage> {
  ...
  Future<void> _yukle() async {
    ...
    try {
      final ayetler = await KuranApi.instance.ayetleriGetir(
        ...
        cuzNo: widget.cuzNo,
        mealEdisyonu: _mealEdisyonu,
      );
      if (mounted && istek == _istekNo) {
        setState(() {
          _ayetler = ayetler;
          _yukleniyor = false;
        });
  ...
  Future<void> _cal(int index) async {
    ...
    try {
      if (_concat == null || !identical(_oynatici.audioSource, _concat) || _oynatici.processingState == ProcessingState.idle) {
      ...
      unawaited(_oynatici.play().catchError((Object e) {
        if (mounted) {
  ...
  void _indexDegisti(int idx) {
    if (!mounted || _ayetler == null || idx < 0 || idx >= _ayetler!.length ||
        _concat == null || !identical(_oynatici.audioSource, _concat)) return;
  ...
  void _listeBitti() {
    if (!mounted || _concat == null || !identical(_oynatici.audioSource, _concat)) return;
    setState(() {
      _caliyor = false;
  ...
  Widget _mealMenu(AppLocalizations l) {
    final secenekler = <Meal>[
      Meal(KuranApi.aktifMealEdisyonu, KuranApi.aktifMealAdi),
      if (l.locale.languageCode == 'tr')
        ...mealler.where((m) => m.id != KuranApi.aktifMealEdisyonu),
    ];
    final secili = _mealEdisyonu ?? KuranApi.aktifMealEdisyonu;
    return PopupMenuButton<String>(
      ...
      itemBuilder: (_) => [
        for (final meal in secenekler)
          CheckedPopupMenuItem<String>(
            value: meal.id,
            checked: meal.id == secili,
            child: Text(meal.ad),
          ),
      ],
    );
            ...
            children: [
              ...
              if (ayet.secdeAyeti)
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: Renkler.altinSabit.withValues(alpha: 0.15),
                    borderRadius: BorderRadius.circular(8),
                  ),
                  child: Text(
                    l.t('sd.secdeAyeti'),
                    style: TextStyle(
                      color: Renkler.altinSabit,
                      fontSize: 9,
                      fontWeight: FontWeight.bold,
                    ),
                  ),
