...
class DilHizmetleri {
  ...
  static Future<Locale> baslat() async {
    ...
    if (kayitli != null && kayitli.isNotEmpty) {
      final kod = kayitli.trim().toLowerCase().split(RegExp('[-_]')).first;
      aktif = desteklenenler.firstWhere(
        (l) => l.languageCode == kod,
        orElse: () => const Locale(varsayilanKod),
      );
    } else {
  ...
  static Future<void> sec(String kod) async {
    final normalizeKod = kod.trim().toLowerCase();
    if (!desteklenenler.any((l) => l.languageCode == normalizeKod)) return;
    final p = await SharedPreferences.getInstance();
    await p.setString(_anahtar, normalizeKod);
    // ValueNotifier, MaterialApp.locale ve Localizations ağacını aynı karede
    // yeniler. Böylece açık olan sayfalar kapanmadan bütün metinler/ikon
    // etiketleri seçilen dile göre yeniden oluşturulur.
    aktifDil.value = Locale(normalizeKod);
  }
